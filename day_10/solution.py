...
def grid_from_string(input_str: str) -> List[List[int]]:
    ...
    """
    return [
        [int(char) for char in line.strip()] for line in input_str.strip().split("\n")
    ]
...
def get_valid_neighbors(
    grid: List[List[int]], row: int, col: int, current_height: int
) -> List[Tuple[int, int]]:
    """
    ...
    for dr, dc in directions:
        ...
        if (
            0 <= new_row < rows
            and 0 <= new_col < cols
            and grid[new_row][new_col] == current_height + 1
        ):
...
# Part 1 Methods
def count_reachable_peaks(grid: List[List[int]], start_row: int, start_col: int) -> int:
    """
...
def solve_part1(grid: List[List[int]]) -> int:
    ...
    return sum(count_reachable_peaks(grid, row, col) for row, col in trailheads)
...
# Part 2 Methods
def count_distinct_trails(grid: List[List[int]], start_row: int, start_col: int) -> int:
    """
...
def solve_part2(grid: List[List[int]]) -> int:
    ...
    """
    trailheads = find_trailheads(grid)
    return sum(count_distinct_trails(grid, row, col) for row, col in trailheads)
